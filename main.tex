...
\textsc{flood} inherits the object extractor and object encoder of C-SWM, which is also used in SCN, making it particularly well-suited for comparison. To enable a more controlled comparison of object discovery approaches with a fixed architecture, we create two ablations of our method that use an SCN-like temporal continuity objective (``Time''), and another that uses an SA-like image reconstruction objective (``Recon''). Since C-SWM already shares the same architecture, it doubles up as the third ablation, representing a dynamics objective (``Dyn''). \jd{Use these names in the table, or change these names.}
...
We will publicly release all datasets and code. Sample frames from all datasets are shown in Fig \jd{??}.
...
We use two key evaluation metrics: \jd{highlight ARI more than MSE, since it is more standard. Also put it first in the tables}
%In order to evaluate the quality of learned object representations, we adopt two evaluation metrics. 
\begin{itemize} [leftmargin=*]
    \item ARI: To evaluate the slot masks as object segmentation masks, we adopt a widely used metric for multi-object segmentation: Adjusted Random Index(ARI)~\cite{ari1,ari2}. ARI measures the similarity of two groupings of pixels, and is permutation invariant. We compare the set of slot masks produced by slot encoder with ground truth segmentation mask(excluding the background mask).
...
In order to measure the above two metrics, we generate ground truth segmentation mask and 2D ground truth location for each object in the observation. For \textbf{REAL}, we labeled
...
\begin{figure}
  \centering
  \includegraphics[width=8cm]{figs/qual_sim.pdf}
  \caption{}
  \label{fig:qual_real}
\end{figure}
\subsection{Results}
\begin{figure}
  \centering
  \includegraphics[width=8cm]{figs/qual_real.pdf}
  \caption{}
  \label{fig:qual_real}
\end{figure}
...
\begin{table}
  ...
  \begin{tabular}{llllll}
    ...
    \cmidrule(r){2-6}
     &  \textbf{MSE(Points)$\downarrow$}  & 100.86 &  99.61  & 455.04 & 396.82    \\
    \cmidrule(r){2-6}
    & \textbf{ARI(\%)$\uparrow$}     &  93.56  & 70.93  & 7.89 & 33.35   \\
    \midrule
    \bottomrule
  \end{tabular}
\end{table}
